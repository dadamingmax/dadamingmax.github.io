...
(function ($, CONFIG) {
    ...
    // hexo-generator-searchdb returns a flat array, while the original
    // Insight plugin expected hexo-generator-json-content's object format.
        ...
        return {
            posts: json.map(function (item) {
                return {
                    title: item.title || '',
                    text: item.content || '',
                    path: String(item.url || '').replace(/^\//, ''),
                    categories: taxonomy(item.categories, 'categories'),
                    tags: taxonomy(item.tags, 'tags')
                };
            }),
            pages: []
        };
